...
import { getMetriquesDedicated, getPerformanceCommerciauxDedie } from '@/app/lib/api/metrics.api';
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-3 rounded-lg border border-blue-200 bg-blue-50 p-4">
        ...
        <p className="text-xs text-blue-700">
          ...
          Elles sont exclues de la vue Globale, de l&apos;Analytique et de la vue Commerciaux pour ne pas fausser les indicateurs.
        </p>
      </div>
      {error && (
      ...
      {loading && !metriques && <p className="text-sm text-gray-500">Chargement...</p>}
      {metriques && (
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <KpiCard title="Messages" value={metriques.totalMessages.toLocaleString('fr-FR')}
              subtitle={`↓ ${metriques.messagesEntrants} entrants · ↑ ${metriques.messagesSortants} sortants`}
              icon={MessageCircle} color="blue" />
            <KpiCard title="Conversations" value={metriques.totalConversations.toLocaleString('fr-FR')}
              subtitle={`${metriques.chatsActifs} actives · ${metriques.chatsEnAttente} en attente`}
              icon={MessageSquare} color="green" />
            <KpiCard title="Non lus" value={metriques.chatsNonLus.toLocaleString('fr-FR')}
              subtitle="Conversations avec messages non lus"
              icon={BellDot} color={metriques.chatsNonLus > 0 ? 'red' : 'slate'} />
            <KpiCard title="Postes dédiés" value={metriques.totalPostes.toLocaleString('fr-FR')}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <KpiCard title="Commerciaux" value={metriques.commerciauxTotal.toLocaleString('fr-FR')}
              subtitle={`${metriques.commerciauxConnectes} connectés`} icon={Users} color="slate" />
            <KpiCard title="Taux de réponse" value={`${metriques.tauxReponse}%`} icon={TrendingUp} color="green" />
            <KpiCard title="Temps réponse moyen"
              value={metriques.tempsReponseMoyen > 0 ? `${Math.round(metriques.tempsReponseMoyen / 60)} min` : '—'}
              icon={Clock} color="blue" />
            <KpiCard title="Conversations fermées" value={metriques.chatsFermes.toLocaleString('fr-FR')} icon={CheckCircle} color="slate" />
          </div>
