...
import { getMediaAssets, getMediaCategories } from '@/app/lib/api/media-asset.api';
    ...
    <div className="flex flex-col gap-4 h-full overflow-y-auto px-4 py-4">
      ...
      >
        {asset.mediaType === 'image' && (
          <img src={asset.publicUrl} alt={asset.name} className="max-h-64 w-full object-contain rounded-xl" />
        )}
        {asset.mediaType === 'video' && (
          <video src={asset.publicUrl} controls className="max-h-64 w-full rounded-xl" />
        )}
      ...
      <div className="space-y-2">
        ...
        <a href={asset.publicUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-xs text-blue-500 hover:text-blue-700">
          ...
          <div className="flex-1 overflow-y-auto p-4 border-r border-gray-100">
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
                {assets.map((asset) => {
                  ...
                  return (
                    ...
                    >
                      <div className="h-20 flex items-center justify-center relative" style={{ backgroundColor: color + '22' }}>
                        {asset.mediaType === 'image' ? (
            ...
            {pages > 1 && (
              <div className="flex justify-center items-center gap-3 pt-4">
                <button disabled={page === 1} onClick={() => setPage((p) => p - 1)} className="text-sm px-3 py-1 border rounded disabled:opacity-40 hover:bg-gray-50">
                  Préc
                </button>
                <span className="text-sm text-gray-500">{page} / {pages}</span>
                <button disabled={page >= pages} onClick={() => setPage((p) => p + 1)} className="text-sm px-3 py-1 border rounded disabled:opacity-40 hover:bg-gray-50">
                  Suiv
                </button>
              </div>
            )}
