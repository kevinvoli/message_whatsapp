...
import {
  Link2, PlusCircle, BarChart3, Copy, Edit, Trash2, ArrowLeft, ExternalLink,
  Smartphone, Monitor, Tablet, HelpCircle, ChevronLeft, ChevronRight,
  Image, Film, Music, FileText, Paperclip, X,
} from 'lucide-react';
import {
  ComposedChart, Bar, Line, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer,
} from 'recharts';
import {
  getCampaignLinks, getCampaignLink, createCampaignLink, updateCampaignLink,
  deleteCampaignLink, getCampaignLinkStats, getCampaignLinkClicks,
  attachMediaAssetToLink, detachMediaAssetFromLink, uploadMediaDirectToLink,
} from '@/app/lib/api/campaign-link.api';
...
function deviceIcon(deviceType: string | null) {
  switch (deviceType) {
    ...
    case 'desktop': return <Monitor className="w-3.5 h-3.5 text-gray-500" />;
    case 'tablet':  return <Tablet className="w-3.5 h-3.5 text-violet-500" />;
    default:        return <HelpCircle className="w-3.5 h-3.5 text-orange-400" />;
  }
}
...
function deviceColor(deviceType: string): string {
  switch (deviceType) {
    ...
    case 'desktop': return 'bg-gray-400';
    case 'tablet':  return 'bg-violet-500';
...
interface LinkFormProps {
  ...
  loading: boolean;
  onSave: (data: { name: string; channel_id: string; predefined_message: string; is_active: boolean }) => Promise<CampaignLink | null>;
  onCancel: () => void;
}
...
function CampaignLinkForm({ channels, initial, loading, onSave, onCancel }: LinkFormProps) {
    ...
    if (result) {
      ...
      if (selectedMedia && !initial?.media_asset_id) {
        try {
          ...
          addToast({ type: 'error', message: e instanceof Error ? e.message : 'Erreur attachement media' });
        }
      }
    }
  };
  ...
  const inputClass = 'w-full rounded-md border border-gray-300 px-3 py-2 text-sm text-gray-800 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500';
  const labelClass = 'mb-1 block text-sm font-medium text-gray-700';

  return (
    <form onSubmit={(e) => void handleSubmit(e)} className="space-y-4">
      <div>
        <label className={labelClass} htmlFor="cl-name">Nom <span className="text-red-500">*</span></label>
        <input id="cl-name" type="text" className={inputClass} placeholder="Ex: Campagne été 2026" value={name} onChange={(e) => setName(e.target.value)} required />
      </div>
      <div>
        <label className={labelClass} htmlFor="cl-channel">Canal <span className="text-red-500">*</span></label>
        <select id="cl-channel" className={inputClass} value={channelId} onChange={(e) => setChannelId(e.target.value)} required>
          <option value="">-- Sélectionner un canal --</option>
          {channels.map((ch) => {
            const label = ch.label || ch.channel_id;
            const phone = ch.phone_number ? ` · ${ch.phone_number}` : '';
            return <option key={ch.id} value={ch.id}>{label}{phone}</option>;
          })}
        </select>
      </div>
      <div>
        <label className={labelClass} htmlFor="cl-message">Message pré-défini</label>
        <textarea id="cl-message" className={inputClass} rows={3} placeholder="Message envoyé automatiquement quand le client clique sur le lien..." value={message} onChange={(e) => setMessage(e.target.value)} />
      </div>
        ...
        {selectedMedia ? (
          <div className="flex items-center gap-3 p-3 bg-white rounded-lg border border-gray-200">
            ...
            <button type="button" onClick={async () => {
              if (mediaLinkId) {
                try { await detachMediaAssetFromLink(mediaLinkId); } catch { /* ignore */ }
              }
              setSelectedMedia(null);
            }} className="p-1.5 rounded hover:bg-gray-100 text-gray-400 hover:text-red-500" aria-label="Detacher le media">
              <X className="w-4 h-4" />
            </button>
          </div>
        ) : (
            ...
            <label className="flex-1 flex items-center justify-center gap-2 px-4 py-3 border-2 border-dashed border-gray-300 rounded-lg text-sm text-gray-500 hover:border-green-400 hover:text-green-600 transition-colors cursor-pointer">
              ...
              <input type="file" className="hidden" onChange={async (e) => {
                ...
                try {
                  ...
                  addToast({ type: 'error', message: err instanceof Error ? err.message : 'Erreur upload' });
                }
              }} />
            </label>
          </div>
      ...
      <div className="flex justify-end gap-2 pt-2">
        <button type="button" onClick={onCancel} className="rounded-md px-4 py-2 text-sm text-gray-600 hover:bg-gray-100" disabled={loading}>
          Annuler
        </button>
        <button type="submit" disabled={loading || !name.trim() || !channelId} className="rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 disabled:opacity-50">
        ...
        <div className="mt-4 rounded-lg border border-green-200 bg-green-50 p-4 space-y-3">
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="min-w-0">
              <p className="text-xs font-semibold text-blue-700 mb-0.5">URL de suivi</p>
              <p className="truncate text-xs font-mono text-gray-700">{savedLink.trackedUrl}</p>
            </div>
            <div className="flex items-center gap-1 flex-shrink-0">
              <button type="button" onClick={() => void copy(savedLink.trackedUrl, 'tracked')}
                className="rounded p-1.5 text-gray-500 hover:bg-green-100" aria-label="Copier l'URL de suivi">
                <Copy className={`w-3.5 h-3.5 ${copying === 'tracked' ? 'text-green-600' : ''}`} />
              </button>
              {savedLink.trackedUrl.startsWith('http') && (
                <button type="button" onClick={() => window.open(savedLink.trackedUrl, '_blank')}
                  className="rounded p-1.5 text-blue-600 hover:bg-blue-50" aria-label="Tester l'URL de suivi">
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <button type="button" onClick={() => void copy(savedLink.directUrl, 'direct')}
              className="rounded p-1.5 text-gray-400 hover:bg-green-100 flex-shrink-0" aria-label="Copier l'URL directe">
          ...
          onSelect={async (asset) => {
            ...
            if (mediaLinkId) {
              try {
                ...
                addToast({ type: 'error', message: e instanceof Error ? e.message : 'Erreur attachement' });
              }
            }
          }}
        />
...
function CampaignLinksList({ onSelectLink }: CampaignLinksListProps) {
  ...
  const handleSave = async (data: { name: string; channel_id: string; predefined_message: string; is_active: boolean }): Promise<CampaignLink | null> => {
    setSaving(true);
    try {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="flex items-center gap-2 text-xl font-semibold text-gray-900">
          <Link2 className="w-5 h-5" /> Liens de campagne
        </h2>
        <div className="flex items-center gap-2">
          <button onClick={() => void load()} className="flex items-center gap-1.5 rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-600 hover:bg-gray-50" title="Rafraîchir">
            <ArrowLeft className="w-4 h-4 rotate-[135deg]" /> Rafraîchir
          </button>
          <button onClick={() => { setEditLink(null); setShowForm(true); }} className="flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700">
            <PlusCircle className="w-4 h-4" /> Nouveau lien
          </button>
        </div>
      </div>
      {showForm && (
        <div className="rounded-xl border border-blue-200 bg-blue-50 p-5 shadow-sm">
          <h3 className="mb-4 text-base font-semibold text-blue-900">{editLink ? 'Modifier le lien' : 'Nouveau lien de campagne'}</h3>
          <CampaignLinkForm channels={channels} initial={editLink} loading={saving} onSave={handleSave} onCancel={() => { setShowForm(false); setEditLink(null); }} />
        </div>
      )}
      {loading ? (
              ...
              {links.map((link) => (
                <React.Fragment key={link.id}>
                  <tr className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-gray-600">{link.channel?.label || link.channelId}</td>
                    ...
                    <td className="px-4 py-3 text-right">
                      ...
                      {link.clickCount > 0 && <span className="ml-1.5 text-xs text-gray-400">({conversionRate(link)})</span>}
                    </td>
                    <td className="px-4 py-3 text-center">
                      {link.isActive ? (
                        ...
                        <span className="inline-flex items-center rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-medium text-gray-600">Inactif</span>
                      )}
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => setExpandedId((prev) => (prev === link.id ? null : link.id))}
                        ...
                        <button onClick={() => onSelectLink(link.id)} className="rounded p-1.5 text-indigo-600 hover:bg-indigo-50" aria-label="Voir les analytics">
                          <BarChart3 className="w-4 h-4" />
                        </button>
                        <button onClick={() => { setEditLink(link); setShowForm(true); }} className="rounded p-1.5 text-blue-600 hover:bg-blue-50" aria-label="Modifier le lien">
                          <Edit className="w-4 h-4" />
                        </button>
                        <button onClick={() => void handleDelete(link.id)} className="rounded p-1.5 text-red-600 hover:bg-red-50" aria-label="Supprimer le lien">
                      ...
                      <td colSpan={8} className="px-4 py-3">
                        <div className="space-y-2">
                          <div className="flex items-center gap-2">
                            ...
                            <code className="flex-1 truncate rounded bg-white border border-blue-200 px-2 py-1 text-xs text-gray-700">{link.trackedUrl || '—'}</code>
                            <button onClick={() => void handleCopyTracked(link.trackedUrl)} className="flex-shrink-0 rounded p-1.5 text-blue-600 hover:bg-blue-100" aria-label="Copier l'URL de suivi">
                              <Copy className="w-3.5 h-3.5" />
                            </button>
                            {link.trackedUrl.startsWith('http') && (
                              <button onClick={() => window.open(link.trackedUrl, '_blank')} className="flex-shrink-0 rounded p-1.5 text-blue-600 hover:bg-blue-100" aria-label="Tester">
                                <ExternalLink className="w-3.5 h-3.5" />
                              </button>
                            )}
                          </div>
                          <div className="flex items-center gap-2">
                            ...
                            <code className="flex-1 truncate rounded bg-white border border-gray-200 px-2 py-1 text-xs text-gray-500">{link.directUrl || '—'}</code>
                            <button onClick={() => { void navigator.clipboard.writeText(link.directUrl).then(() => addToast({ type: 'success', message: 'URL directe copiée.' })); }}
                              className="flex-shrink-0 rounded p-1.5 text-gray-400 hover:bg-gray-100" aria-label="Copier l'URL directe">
                              <Copy className="w-3.5 h-3.5" />
                            </button>
                          </div>
                        </div>
...
function CampaignLinkAnalytics({ linkId, onBack }: CampaignLinkAnalyticsProps) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <button onClick={onBack} className="flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm text-gray-600 hover:bg-gray-100" aria-label="Retour à la liste">
        ...
        <>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-2 text-sm text-gray-600">
              <input type="date" value={from} onChange={(e) => setFrom(e.target.value)} className="rounded border border-gray-300 px-2 py-1 text-xs focus:outline-none" aria-label="Date de début" />
              <span>→</span>
              <input type="date" value={to} onChange={(e) => setTo(e.target.value)} className="rounded border border-gray-300 px-2 py-1 text-xs focus:outline-none" aria-label="Date de fin" />
            </div>
          </div>
                ...
                {stats.clicks_by_device.map((item) => (
                  <div key={item.device_type} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 rounded-full bg-gray-100 h-2">
                      <div className={`h-2 rounded-full ${deviceColor(item.device_type)}`} style={{ width: `${(item.count / maxDeviceCount) * 100}%` }} />
                    </div>
              ...
              {clicks.map((click) => (
                <tr key={click.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2 text-center">
                    {click.converted ? (
                      <span className="inline-flex items-center gap-1 rounded-full bg-green-100 px-2.5 py-0.5 text-xs font-medium text-green-700">Converti</span>
                    ) : (
                      <span className="inline-flex items-center rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-medium text-gray-500">En attente</span>
                    )}
                  </td>
                  <td className="px-4 py-2 text-xs text-gray-500">{click.convertedAt ? formatDate(click.convertedAt) : '—'}</td>
                  <td className="px-4 py-2 text-xs">
                    {click.chatId ? <span className="text-blue-600 font-medium">Voir</span> : <span className="text-gray-300">—</span>}
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        {!loadingClicks && (
          <div className="flex items-center justify-between border-t border-gray-100 px-4 py-3">
            <button onClick={() => setClickPage((p) => Math.max(1, p - 1))} disabled={clickPage === 1}
              className="flex items-center gap-1 rounded px-3 py-1.5 text-xs text-gray-600 hover:bg-gray-100 disabled:opacity-40" aria-label="Page précédente">
              <ChevronLeft className="w-3.5 h-3.5" /> Précédent
            </button>
            <span className="text-xs text-gray-500">Page {clickPage}</span>
            <button onClick={() => setClickPage((p) => p + 1)} disabled={clicks.length === 0}
              className="flex items-center gap-1 rounded px-3 py-1.5 text-xs text-gray-600 hover:bg-gray-100 disabled:opacity-40" aria-label="Page suivante">
...
export default function CampaignLinksView() {
  ...
  if (selectedLinkId) {
    return <CampaignLinkAnalytics linkId={selectedLinkId} onBack={() => setSelectedLinkId(null)} />;
  }
