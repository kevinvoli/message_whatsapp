      ...
      {(['heure', 'jour'] as const).map((g) => (
        <button key={g} onClick={() => onChange(g)}
          className={[
            ...
            value === g ? 'bg-white text-indigo-600 border border-indigo-200 shadow-sm' : 'text-gray-500 hover:text-gray-700',
          ].join(' ')}
        >
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
      <div className="flex items-center justify-between mb-2">
        ...
        <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${colorClass}`}>{icon}</div>
      </div>
...
function KpiGridConversations({ stats, granularite }: { stats: TraficConversationsResponse['statistiques']; granularite: 'heure' | 'jour' }) {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      <KpiCard title="Total conversations" value={stats.total.toLocaleString('fr-FR')} subtitle="Ouvertes sur la période" icon={<MessagesSquare size={16} />} colorClass="bg-indigo-50 text-indigo-600" />
      <KpiCard title="Actives" value={stats.actives.toLocaleString('fr-FR')} subtitle={`${stats.taux_actives}% du total`} icon={<MessageCircle size={16} />} colorClass="bg-green-50 text-green-600" />
      <KpiCard title="Fermées" value={stats.fermees.toLocaleString('fr-FR')} subtitle={`Taux clôture : ${stats.taux_cloture}%`} icon={<CheckCircle size={16} />} colorClass="bg-blue-50 text-blue-600" />
      <KpiCard title="En attente" value={stats.en_attente.toLocaleString('fr-FR')} subtitle="En attente de traitement" icon={<Clock size={16} />} colorClass="bg-amber-50 text-amber-600" />
      <KpiCard title="Moy. / heure" value={`${stats.moy_par_heure} conv/h`} subtitle={`${stats.unites_actives} créneaux actifs`} icon={<Zap size={16} />} colorClass="bg-yellow-50 text-yellow-600" />
      <KpiCard title="Moy. / jour" value={`${stats.moy_par_jour} conv/j`} subtitle={`Sur ${stats.nb_jours} jour${stats.nb_jours > 1 ? 's' : ''}`} icon={<CalendarDays size={16} />} colorClass="bg-purple-50 text-purple-600" />
      <KpiCard title="Créneau de pic" value={picLabel} subtitle={`${stats.conversations_pic} conversations`} icon={<TrendingUp size={16} />} colorClass="bg-rose-50 text-rose-600" />
      <KpiCard title="Taux clôture" value={`${stats.taux_cloture}%`} subtitle="Conversations fermées" icon={<BarChart2 size={16} />} colorClass="bg-teal-50 text-teal-600" />
    </div>
  );
}

function ConvTooltip({ active, payload, label }: { active?: boolean; payload?: Array<{ value: number; name: string }>; label?: string }) {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg p-3 shadow-lg text-sm">
      ...
      <p className="text-gray-700 border-t mt-2 pt-2 font-medium">Total : {actives + fermees}</p>
    </div>
  );
}

function ConversationBarChart({ points, granularite, selectedPeriod }: { points: TraficConversationsPoint[]; granularite: 'heure' | 'jour'; selectedPeriod: string }) {
  const labels: Record<string, string> = { week: '7 derniers jours', month: '30 derniers jours', year: '12 derniers mois' };
  const title = granularite === 'heure'
    ? (selectedPeriod === 'today' ? "Conversations heure par heure — aujourd'hui" : `Moyenne horaire — ${labels[selectedPeriod] ?? selectedPeriod}`)
    : 'Conversations par jour de la semaine';
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
      <h3 className="text-base font-semibold text-gray-800 mb-4">{title}</h3>
      <ResponsiveContainer width="100%" height={320}>
        <BarChart data={points} margin={{ top: 10, right: 20, left: 0, bottom: 5 }} barGap={2}>
          <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
          <XAxis dataKey="label" tick={{ fontSize: 11, fill: "#6b7280" }} interval={granularite === 'heure' ? 1 : 0} />
        ...
        {items.map((item) => (
          <div key={item.label}>
            ...
            <div className="w-full bg-gray-100 rounded-full h-2">
              <div className={`${item.color} h-2 rounded-full transition-all duration-500`} style={{ width: `${Math.min(item.pct, 100)}%` }} />
            </div>
          </div>
        ))}
      </div>
...
function TopCreneauxConversations({ points, total, granularite }: { points: TraficConversationsPoint[]; total: number; granularite: 'heure' | 'jour' }) {
          ...
          <tbody>
            {top5.map((p, idx) => (
              <tr key={p.index} className="border-b border-gray-50 hover:bg-gray-50">
                <td className="py-2 font-medium text-gray-800">{idx === 0 ? '🔥 ' : '    '}{p.label}</td>
                ...
                <td className="py-2 text-right text-gray-500">{total > 0 ? Math.round((p.total / total) * 100) : 0}%</td>
              </tr>
            ))}
          </tbody>
        </table>
...
export default function ConversationsTrafficTab({
  loading, data, error, granularite, onGranulariteChange, selectedPeriod,
}: ConversationsTrafficTabProps) {
  const isEmpty = data !== null && data.statistiques.total === 0;

  return (
    <div className="space-y-6">
      {loading && !data && <div className="flex items-center justify-center h-48"><Spinner /></div>}
      ...
      {isEmpty && (
        <div className="bg-gray-50 border border-gray-200 rounded-xl p-10 flex flex-col items-center justify-center text-center gap-3">
          ...
          <p className="text-xs text-gray-400">Essayez une plage de dates différente ou vérifiez que les canaux sont actifs.</p>
        </div>
      )}
      {data && !isEmpty && (
        <>
          ...
          <ConversationBarChart points={data.points} granularite={granularite} selectedPeriod={selectedPeriod} />
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <RepartitionStatuts stats={data.statistiques} />
            <TopCreneauxConversations points={data.points} total={data.statistiques.total} granularite={granularite} />
          </div>
        </>
      )}
    </div>
