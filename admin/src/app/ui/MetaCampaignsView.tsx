...
import { getCampagnesMeta } from '@/app/lib/api/metrics.api';
import { resolveMediaUrl } from '@/app/lib/utils';
import { MetaAdKpiRow } from '@/app/lib/definitions';
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-end gap-4 bg-white border border-gray-200 rounded-xl shadow-sm p-4">
        ...
        <button onClick={() => { setRows(null); void load(dateFrom, dateTo); }} disabled={!dateFrom || !dateTo || loading}
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        {!loading && !error && rows !== null && rows.length === 0 && (
          <p className="text-sm text-gray-400 text-center py-12">Aucune campagne sur cette période.</p>
        )}
        {!loading && !error && rows !== null && rows.length > 0 && (
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <p className="font-medium text-gray-900 truncate max-w-xs">{row.headline ?? '—'}</p>
                          <p className="text-xs text-gray-400 font-mono truncate max-w-xs">
                            {row.source_id.length > 20 ? `${row.source_id.slice(0, 20)}…` : row.source_id}
                          </p>
                        </div>
                      </div>
                    </td>
                    <td className="py-3 px-3 text-right font-semibold text-gray-900">{row.total_conversations.toLocaleString('fr-FR')}</td>
                    <td className="py-3 px-3 text-right text-gray-600">{row.conversations_closed.toLocaleString('fr-FR')}</td>
                    <td className="py-3 px-3 text-right">
                      <span className={`font-medium ${row.conversion_rate >= 60 ? 'text-green-600' : row.conversion_rate >= 30 ? 'text-yellow-600' : 'text-red-500'}`}>
                        {row.conversion_rate}%
                      </span>
                    </td>
                    <td className="py-3 px-3 text-right text-gray-600">{row.avg_messages_per_chat.toLocaleString('fr-FR')}</td>
                    <td className="py-3 px-3 text-right text-gray-600">{formatSeconds(row.avg_first_response_s)}</td>
                    <td className="py-3 px-3 text-right text-gray-500 text-xs">{formatDateShort(row.first_seen)}</td>
                    <td className="py-3 px-4 text-right text-gray-500 text-xs">{formatDateShort(row.last_seen)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
