...
import { getTraficHoraire, getTraficConversations } from '@/app/lib/api/metrics.api';
...
function CustomTooltip({ active, payload, label, isMoyenne }: { active?: boolean; payload?: Array<{ value: number }>; label?: string; isMoyenne: boolean }) {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg p-3 shadow-lg text-sm">
      ...
      <p className="text-gray-700 border-t mt-2 pt-2 font-medium">Total : {inVal + outVal}</p>
    </div>
  );
}
...
function TrafficBarChart({ points, isMoyenne, nbJours, granularite, selectedPeriod }: { points: TraficPoint[]; isMoyenne: boolean; nbJours: number; granularite: 'heure' | 'jour'; selectedPeriod: string }) {
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
      <div className="mb-4">
        <h3 className="text-base font-semibold text-gray-800">{buildChartTitle(granularite, isMoyenne, nbJours, selectedPeriod)}</h3>
        {isMoyenne && (
          <p className="text-xs text-amber-600 bg-amber-50 border border-amber-200 rounded px-2 py-1 inline-flex items-center gap-1 mt-1">
            <Info size={12} /> Valeurs = moyenne par jour sur {nbJours} jour{nbJours > 1 ? 's' : ''}
          </p>
        )}
      </div>
      <ResponsiveContainer width="100%" height={320}>
        <BarChart data={points} margin={{ top: 10, right: 20, left: 0, bottom: 5 }} barGap={2}>
          ...
          <YAxis tick={{ fontSize: 11, fill: "#6b7280" }} width={40} allowDecimals={isMoyenne} />
        ...
        {tranches.map((t) => (
          <div key={t.label}>
            <div className="flex items-center justify-between mb-1">
              <span className="text-sm text-gray-700">{t.emoji} {t.label} <span className="text-xs text-gray-400">({t.heures})</span></span>
            ...
            <div className="w-full bg-gray-100 rounded-full h-2">
              <div className="bg-indigo-500 h-2 rounded-full transition-all duration-500" style={{ width: `${Math.min(t.pct, 100)}%` }} />
            </div>
          </div>
        ))}
      </div>
              ...
              <div className="flex items-center justify-between">
                ...
                {granularite === 'jour' && selectedPeriod === 'today' && (
                  <p className="text-xs text-amber-600 flex items-center gap-1">
                    <Info size={12} /> Mode jour indisponible pour aujourd&apos;hui — données sur 7 jours
                  </p>
                )}
              </div>
              <KpiGrid stats={data.statistiques} />
              <TrafficBarChart points={data.points} isMoyenne={isMoyenne} nbJours={data.meta.nb_jours} granularite={granularite} selectedPeriod={selectedPeriod} />
