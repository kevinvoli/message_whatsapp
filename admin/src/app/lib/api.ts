...
export async function updateCommercial(
  id: string,
  payload: { name?: string; email?: string; password?: string; poste_id?: string | null; is_active?: boolean; allowOutsideHours?: boolean; bypassRestrictions?: boolean },
): Promise<Commercial> {
...
export async function disconnectCommercial(id: string): Promise<DisconnectCommercialResponse> {
    const response = await fetch(`${API_BASE_URL}/users/${id}/disconnect`, {
        method: 'POST',
        credentials: 'include',
    });
    return handleResponse<DisconnectCommercialResponse>(response);
}
...
export async function getClients(
    ...
    cursor?: string,
    search?: string,
): Promise<{ data: Client[]; nextCursor: string | null; hasMore: boolean }> {
    ...
    return handleResponse<{ data: Client[]; nextCursor: string | null; hasMore: boolean }>(response);
}

export async function getClientsOffset(limit = 50, offset = 0, search?: string): Promise<{ data: Client[]; total: number }> {
...
export async function getChats(
    ...
    const result = await handleResponse<{ data: Array<Partial<WhatsappChat> & {
      ...
      status?: string;
    }>; total: number; totalAll?: number; totalActifs?: number; totalEnAttente?: number; totalUnread?: number; totalFermes?: number }>(response);
    return {
...
/**
 * Recupere le trafic messages agrege par heure ou par jour de semaine
 */
...
export async function updateMessageRestrictionConfig(config: MessageRestrictionConfig): Promise<MessageRestrictionConfig> {
    const response = await fetch(`${API_BASE_URL}/admin/message-restrictions`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
...
export async function getQuizQuestions(filters?: { categoryId?: string; search?: string; activeOnly?: boolean }): Promise<QuizQuestion[]> {
  const params = new URLSearchParams();
...
export async function duplicateQuizSession(id: string, targetDates: string[]): Promise<{ created: string[]; skipped: string[] }> {
