...
const PROVIDER_CONFIG: Record<Provider, {
  ...
  whatsapp:  { label: 'WhatsApp',  bgColor: 'bg-emerald-50', textColor: 'text-emerald-700', iconColor: 'text-emerald-500', avatarBg: 'bg-emerald-100', avatarText: 'text-emerald-700', Icon: WhatsAppIcon  },
  messenger: { label: 'Messenger', bgColor: 'bg-blue-50',    textColor: 'text-blue-700',    iconColor: 'text-blue-500',    avatarBg: 'bg-blue-100',    avatarText: 'text-blue-700',    Icon: MessengerIcon },
  instagram: { label: 'Instagram', bgColor: 'bg-purple-50',  textColor: 'text-purple-700',  iconColor: 'text-purple-500',  avatarBg: 'bg-purple-100',  avatarText: 'text-purple-700',  Icon: InstagramIcon },
  telegram:  { label: 'Telegram',  bgColor: 'bg-sky-50',     textColor: 'text-sky-700',     iconColor: 'text-sky-500',     avatarBg: 'bg-sky-100',     avatarText: 'text-sky-700',     Icon: TelegramIcon  },
};
...
export function ProviderBadge({ chatId, showLabel = false, className = '' }: ProviderBadgeProps) {
  const provider = getProviderFromChatId(chatId);
  const { label, bgColor, textColor, iconColor, Icon } = PROVIDER_CONFIG[provider];

  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full px-1.5 py-0.5 text-[11px] font-medium ${bgColor} ${textColor} ${className}`}
      title={label}
    >
